...
unsigned char read_ppi(unsigned char),echostring[100];
...
/*flash*/ unsigned int dc[]={0,0,21,21,14,12,17,12,18,18,20,10,6,6,5,2};
...
/* eeprom*/ int lockpos=150,molpos=170,s2pos=250,s1pos=700,limitrmo=800;
/* eeprom*/ unsigned char lockspeed=60,lockpre=80,molprop=60,molpros=120,s2=70,p2=60,s1=120,p1=60;
/* eeprom*/ unsigned char memvar[132][24];
/* eeprom*/ unsigned int maxruler1=255,maxruler2=300,maxruler3=300,marrid,whour=100,ccunter;
...
u16 LOADMEM_TO_VAR(u16);
 ...
 void controll(void)//ok1
  ...
  char tempb=0;//st
		...
		for(ccc=0;ccc<5;ccc++)
  {                                                    
      if(ehteyatkey)ttttt=var2[8][16]<<1;else ttttt=var[8][ccc]<<1;
  ...
  if(var[8][0]>var[8][6]) temptemp=(var[8][0]-var[8][6])<<1; else temptemp=0;
  if(  (tempreture[0]<((var[8][0]+var[8][5])<<1)) && (tempreture[0]>(temptemp)))hit1ok=1;else hit1ok=0;
  if(var[8][0]<40)hit1ok=1;
 
  if(var[8][1]>var[8][6]) temptemp=(var[8][1]-var[8][6])<<1; else temptemp=0;
  if(  (tempreture[1]<((var[8][1]+var[8][5])<<1)) && (tempreture[1]>(temptemp)))hit2ok=1;else hit2ok=0;
  if(var[8][1]<40)hit2ok=1;  

  if(var[8][2]>var[8][6])temptemp=(var[8][2]-var[8][6])<<1;else temptemp=0;
  if(  (tempreture[2]<((var[8][2]+var[8][5])<<1)) && (tempreture[2]>(temptemp)))hit3ok=1;else hit3ok=0;
  if(var[8][2]<40)hit3ok=1;

  if(var[8][3]>var[8][6])temptemp=(var[8][3]-var[8][6])<<1;else temptemp=0;
  if(  (tempreture[3]<((var[8][3]+var[8][5])<<1)) && (tempreture[3]>(temptemp)))hit4ok=1;else hit4ok=0;
  if(var[8][3]<40)hit4ok=1;

  if(var[8][4]>var[8][6])temptemp=(var[8][4]-var[8][6])<<1;else temptemp=0;
  if(  (tempreture[4]<((var[8][4]+var[8][5])<<1)) && (tempreture[4]>(temptemp)))hit5ok=1;else hit5ok=0;
   if(var[8][4]<40)hit5ok=1;
  ...
  if(hit1ok && hit2ok && hit3ok && hit4ok)hitok=1; else hitok=0; 
   ...
   if(var[8][4]<40)tempreture[4]=var[8][4]+8;
  ...
  if(tempreture[0]<temptemp)
     {
     if(var[8][0]>40)tempr1=1;
     }
 if(var[8][1]>var[8][6])temptemp=(var[8][1]-var[8][6])<<1;else temptemp=0;
  if(tempreture[1]<temptemp)
     {
     if(var[8][1]>40)tempr1=1;
     }
                       
  if(var[8][2]>var[8][6])temptemp=(var[8][2]-var[8][6])<<1;else temptemp=0;
  if(tempreture[2]<temptemp)
     {
     if(var[8][2]>40)tempr1=1;
     }
                         
 if(var[8][3]>var[8][6])temptemp=(var[8][3]-var[8][6])<<1;else temptemp=0;
  if(tempreture[3]<temptemp)
     {
     if(var[8][3]>40)tempr1=1;
     }                   
...
if(pri1  && lockkey1 )
 {                                 
  ...
  if(hitok)
   {
    memseg=0;
     if(ruler2>=ruler2p1 && ristate<=1) 
       {              
       memseg=1;
        tttt=var[4][0]%100;//marhale 1
        speed=tttt;
        tttt=var[4][0]/100;
        per=tttt;
        setout5;                   
        ...
        timemax=var[4][1];
        if(ritime==var[4][1]){ristate=2;ritime=0;}
       }else
               if(ruler2>=ruler2p2 &&  ristate<=2) 
                {        
                memseg=2;
        tttt=var[4][3]%100;
          speed=tttt;
        tttt=var[4][3]/100;                    //marhale  2
        per=tttt;
                setout5;
                clrout14;
        timemax=var[4][4];
        if(ritime>=var[4][4]){ritime=0;ristate=3;}
                                   ...
                                   if(ristate<=3)
                                  {
                                    ...
                                    clrout14;                                    
                                         timemax=var[4][6];
                                     if(ritime>=var[4][6]){timemax=0;ritime=0;ristate=4;finish=1;timer=var2[3][10]; tempp=1;  speed=var[2][1];per=var[2][0];pri1=0;clrout5;} 
  ...
  //    tempp=1;   
      ...
      //pri1=0;      
        //errorcode=10;
     if(hightemp)errorcode=11;else 
     if(lowtemp)errorcode=10;else errorcode=10;
    // errorcode=23;
        }
 ...
 {                           
  ...
  if(hitok)
   { 
     ...
     if((!mavad2 && ruler2<=ruler2p4) || ( mavad2 && ritime<(var2[3][8]) )) 
       {
        tempp=0;
        speed=var[3][1];//mavadgiri 
        per=var[3][0];
        setout6;                                  
        setout14; 
       }else
                {          
                if(ritime<(var2[3][0]*10))finish=1;else errorcode=13;
                 timer=var2[3][11];
                 speed=0;per=0;
                 pbri1=0;
                 clrout6;
                ...
                clrout14;
        ...
        }else   
                         {
                               ...
                               speed=0;
                                ...
                                clrout6;
                                clrout14;
      if(hightemp)errorcode=11;
      if(lowtemp)errorcode=10;
...
if((pepm1   &&  lockkey1))
 {                           
  action=0;
  ritime2++;
 if( ruler1>=ruler1p6  && ritime2<(100*var2[6][8]))
   {
    setout13;                 
...
if((pci1   &&  lockkey1))
 ...
 if((LCI || timer) && !LEB    && ((ruler1+10)>=ruler1p6) )
   {
  //  if(timer) setout10;    
   if(timer && (var2[6][17])){ if(timer<(var[6][2]/2)){setout10;clrout23;}else {setout23;clrout10;}} else {setout10;clrout23;}                  
    speed=var[6][1];
    per=var[6][0];
     ...
     if(LEB)errorcode=14;else errorcode=0;
     if(!LCI) {finish=1;timer=var2[0][20]+1;}
//        else
  //   if(LEB)errorcode=14;else errorcode=0;
...
if((pco1   &&  lockkey1))
 ...
 if(LCO || timer )
   {
   if(timer  && (var2[6][17])){ if(timer<(var[6][5]/2)){setout11;clrout24;}else {setout24;clrout11;}} else {setout11;clrout24;}                
    speed=var[6][4];                                                       
    per=var[6][3];
...
if((pef1   &&  lockkey1))
 ...
 if(!LCO && ((((LEF && RULER_SELECT1) || (ruler3p3>ruler3 && !RULER_SELECT1))  || timer) && ((ruler1+31)>=ruler1p6)) )
   {
    setout8;                 
    speed=var[5][1];
    per=var[5][0];
     ...
     clrout8;                        
      ...
      per=0;
               	     ...
               	     if(LCO)errorcode=9;else{ finish=1;errorcode=0;}
    // if(!LEF || !timer)finish=1;
     timer=3;
     }
   }
   else 
...
if((puf1   &&  lockkey1))
 {                           
  action=0;
                //                      if((LUB || var2[2][5]) && (ruler3p2<ruler3 || !var2[2][5]))

    if(!LMC && (LUF && RULER_SELECT) || (ruler3p1<ruler3 && !RULER_SELECT))timer=var2[2][12];
 if((!LMC && ((LUF && RULER_SELECT) || (ruler3p1<ruler3 && !RULER_SELECT))) || timer)
   {
    setout3;
    setout17;
    setout14;
    ...
    //  per=0;
     ...
     if( LMC )errorcode=40;else errorcode=0;
     if(   !LMC ){finish=1;
         setout3;
         setout17;
         setout14;
...
if((pub1  && lockkey1))
                               {                           
                                 action=0;
                                 if((LUB && RULER_SELECT) || (ruler3p2>ruler3 && !RULER_SELECT) && timer)
                          //       if(ruler3p2<ruler3 && LUB && timer )
              		    {
                		speed=var[2][3];
                        	per=var[2][4];
                                			setout4;
...
if((pgb1  && lockkey1))
                               {                           
                                 ...
                                 if(((ruler1+11)>=ruler1p6) && (ritime1)<(var2[7][0]))
                                 		    {
		                                 speed=var[7][1];
		                                 per=var[7][0];
                                                             setout12;
...
if((peb1  && lockkey1))
                               {                           
                                     action=0;
                                    // if(((LEB || !RULER_SELECT) && (ruler3p4<ruler3 || RULER_SELECT) || timer ))
                                     if(((LEB ) || timer ))
                                              {
                                             per= var[5][3];
                                           speed=var[5][4];
...
/**************** RMC *****************/
if(action)
if((pmc1  && lockkey1))                                     //GIREH BASTEH
 {                           
  action=0;
  if(LGR && temp111) {setout23;speed=var2[6][11];per=var2[6][10];} else
  {
  if(!LGR && !LGF && LMC && ( ( !LEB) || !var[5][8] ) && (!LCI  || !var2[6][16]) )
//  if(1)                                         
   {             
   temp111=0;
    clrout23;
     if(ruler1<=ruler1p3)                           //200
       {
        clrout9;
        speed=speedv19;
        per=perv19;
        if(yekst){yekst=0;timer=var2[0][12];}       
        clrout14;         
//        if(!i15){setout17;setout14;speed=var[0][5]%100;per=var[0][5]/100;} 
        if(i15 || timer ){clrout1;setout15;speed=var[7][4];per=var[7][3];}else {clrout15;setout17;setout14;speed=var[0][5]%100;per=var[0][5]/100;}                
                                              
        //clrout9;
        }else
         if(ruler1<=ruler1p2+4)                      //400
           {
       ...
       //    if(i15)setout17;
           ...
           if(!i15) clrout15; 
              ...
              if(ruler1<=ruler1p1)           //500
              {
             if(var2[5][8])  setout9;
               speed=var2[0][7];//speedv2;
               per=var2[0][6];//perv2;
       ...
       if(!LMC){
       pmc1=0;finish=1;timer=var2[2][12];clrout9;}
      else
      {
      
       if(LGR){errorcode=3;}
       else if(LGF)errorcode=4;
        
        else
         if((LEB))errorcode=14;
      //   if(((LEB || !RULER_SELECT) && (ruler3p4<ruler3 || RULER_SELECT)))errorcode=14; 
        
         else 
          if(LCI && var2[6][16])errorcode=24;
        else errorcode=0;
       clrout17;
       clrout14;  
       }
...
if(action &&  (!timer || !yekst) )
{
if((pmo1 && lockkey1 ) )//GERE BAZ
 {                           
   
   
    if(ruler1>ruler1p7  && var2[6][17])setout13;   
    if(yekst){timer=var2[1][4];yekst=0;}
    action=0;
 //   clrout24;
    clrout3;clrout17;clrout14;
  if(i17){timer=var2[1][4];setout18;speed=var2[1][3];per=var2[1][2];}
  
     else
     {
     clrout18;
      if(i16 || timer){setout16;speed=var[7][6];per=var[7][5];delaytaghe=100;}
     else
    if(!delaytaghe)
  {
  clrout16;
  if( ruler1<(ruler1p6+3) )            //960  
   {
     if(ruler1<=ruler1p4) 
       {
        speed=speedv5;
     ...
     {delaytaghe--;clrout16;per=0;speed=0;}
...
{
 if(pmo1 && LGO &&semiauto)
    {
      if(i21) {speed=var2[6][15];per=var2[6][14];}
        ...
        if(ritime>=var[4][4]){ritime=0;ristate=3;}
                                   ...
                                   if(ristate<=3)
                                  {
                                      memseg=3;
                                       ...
                                       clrout14;                  // marhale  3
                                          timemax=var[4][6];
                                     if(ritime>=var[4][6]){timemax=0;ritime=0;ristate=4;finish=1;timer=3;} 
...
if((pepm   &&  lockkey))
 {                           
  action=0;
  ritime2++;
 if( ruler1>=ruler1p6  && ritime2<5000)
   {
    setout13;                 
 ...
 if(ruler1<= ruler1p3 )
   {
    ...
    setout15;speed=var[7][4];per=var[7][3];
 ...
 if((LCI || timer) && !LEB    && ((ruler1+10)>=ruler1p6) )
   {
    ...
    per=var[6][0];
     ...
     if(LEB)errorcode=14;else if(!((ruler1+10)>=ruler1p6))errorcode=9; else errorcode=0;
     if(!LCI) {finish=1;timer=var[0][8]+1;}
//        else
  //   if(LEB)errorcode=14;else errorcode=0;
...
if((pef   &&  lockkey))
 ...
 if(!LCO && ( ((LEF && RULER_SELECT1) || (ruler3p3>ruler3 && !RULER_SELECT1)) || timer ) && ((ruler1+11)>=ruler1p6) )
   {
    setout8;                 
    speed=var[5][1];
    per=var[5][0];
     ...
     clrout8;                        
      ...
      per=0;
               	     ...
               	     if(LCO || ((ruler1+11)< ruler1p6))errorcode=9;else{ finish=1;errorcode=0;pef=0;}
    // if(!LEF || !timer)finish=1;
     timer=3;
     }
   }
 ...
 if((!LMC && ((LUF && RULER_SELECT) || (ruler3p1<ruler3 && !RULER_SELECT))) || timer )
   {
    ...
    //  per=0;
     ...
     if( LMC )errorcode=40;else errorcode=0;
    // if( !LUF &&  !LMC){finish=1;
     if( !LMC && !((LUF && !RULER_SELECT1) || (ruler3p1<ruler3 && RULER_SELECT1))){finish=1;
...
if((pub  && lockkey))
  ...
  //                        if((LUB || RULER_SELECT) && (ruler3p2>ruler3 || !RULER_SELECT))
                          if((LUB && RULER_SELECT) || (ruler3p2>ruler3 && !RULER_SELECT))
...
/**************** RMC *****************/
if(action)
if((pmc  && lockkey))                                     //GIREH BASTEH
 {                           
  action=0;
  if((!LGR || ehteyatkey) && (!LGF || ehteyatkey) && LMC && ( ( !LEB) || !var[5][8] ) && (!LCI || !var2[6][16]) )
//  if(1)
   {
     if(ruler1<=ruler1p3)                           //200
       {
         if(ehteyatkey)speed=speedehteyat; else speed=speedv19;
        per=perv19;
//        setout1;       
       // clrout14;         
       // if(!i15){setout17;setout14;if(ehteyatkey)speed=speedehteyat; else speed=var[0][5]%100;per=var[0][5]/100;} 
       // if(i15){setout15;speed=var[7][4];per=var[7][3];}else clrout15;                
       ...
       if(yekst){yekst=0;timer=var2[0][12];}       
        ...
        if(i15 || timer ){clrout1;setout15;speed=var[7][4];per=var[7][3];}else {clrout15;setout17;setout14;speed=var[0][5]%100;per=var[0][5]/100;}                
         ...
         if(ruler1<=ruler1p2)                      //400
           ...
           if(ehteyatkey )speed=speedehteyat; else 
                ...
                clrout14 ;
                 ...
                 if(!i15) clrout15;
                   ...
                   if(var2[5][8])setout9;
                    ...
                    if(ehteyatkey)speed=speedehteyat; else speed=speedv4;
                   per=perv4;
                  setout1;
                 setout14;
                 if(!i15)clrout15;
       ...
       if(!LMC){
       pmc=0;finish=1;timer=var[2][7];}
      else
      {
       if(LGR && !ehteyatkey){errorcode=3;/*//st read(0);*/}
       else if(LGF && !ehteyatkey)errorcode=4;
...
if(action)
if((pmo && lockkey) )//GERE BAZ
 {                           
    action=0;
  if(i17){setout18;speed=var2[1][3];per=var2[1][2];}
  
     else
     {
     clrout18;
      if(i16 || timer){setout16;speed=var[7][6];per=var[7][5];}
     else
  
  {
  clrout16;
  if( ruler1<ruler1p6 )            //960  
   {
     if(ruler1<=ruler1p4) 
       {
         if(ehteyatkey)speed=speedehteyat; else speed=speedv5;
        per=perv5;
        setout2;
        clrout14;
        }else
         if(ruler1<=ruler1p5) 
           ...
           if(ehteyatkey)speed=speedehteyat; else speed=speedv6;
           per=perv6;
          setout2;
          setout14;
              ...
              if(1) 
              {
               if(ehteyatkey)speed=speedehteyat; else speed=speedv7;
               per=perv7;
               setout2;
               clrout14;     
                  ...
                  if(ruler1<=s1pos) {
                if(ehteyatkey)speed=speedehteyat; else    speed=s1;
                   per=p1;
                   setout2;
                   clrout14;
                    ...
                    {
                      if(ehteyatkey)speed=speedehteyat; else speed=50;
                     per=95;
                     setout2;
                     clrout14;
 ...
 if(LMC  && var2[7][16])
  {
     action=0;
      ...
      else {clrout22;if(i3){speed=0;per=0;}}
  
  
  
  }else {per=0;speed=0;clrout22;clrout21;}
...
//makebuf();
if(color_refresh<80)color_refresh++; else color_refresh=0;
old[color_refresh]+=1;
...
if(!errorcode)cpp(0,1,0,keycode,0);
else cpp(0,1,0,errorcode,0);
...
if(state2==4 ||state2==3 || state2==5){ftemp=ritime;cpp(1,8,8,ftemp/10,1);}
 ...
 if(!RULER_SELECT1){    cpp(1,38,16,ruler3m,0);    }else{    cpp(1,38,16,ruler3m,9);    }
 if(!RULER_SELECT){        cpp(1,38,6,ruler3m,0);}else{    cpp(1,38,6,ruler3m,9);    }
...
if(page && page!=14)cpp(1,0,21,temp,0);
//cpp(1,0,21,ruler2,0);
...
if(oldpageco!=page)
 ...
 if(beepcounter){lcd_beep(beepcounter);beepcounter=0;}
...
if(receive_data[0]==0xa5 &&
  ...
  if(!serial_data_valid){touch_command_pointer=0;receive_data[0]=0;}
USART_SendData(USART2,0);//afst
//st UDR0=0;
//if(!(cheshmak &0xf))
        ...
        if((opcode%10)==7 )ccunter=0;
          ...
          if(m==3)
            {    
             ...
             if(1){
                            m=opcode%10;
                            if(m==0){maxruler1=varcode;}
                            if(m==1){maxruler2=varcode;}
                            if(m==2){maxruler3=varcode;}
                            if(m==4){
                                            if(temp==0)
                                              {
                                               t1cul=varcode;
          ...
          if(m==1)
            {
             if(1){
                            m=opcode%10;
                            if(m==0){if(varcode==pass1 || varcode==2468){
                            if(oprand==0)lock=0;else lock=1;
                                                                              
                                                                              
                                                                              }}
                                                                             
                             if(m==1 && lock==1){pass1=varcode;}
                            
                             if(m==2){if(varcode==pass2){
                                                                              if(oprand==0)lock1=0;else lock1=1;
                                                                              
                                                                              
                                                                              }}
                                                                             
                             if(m==3 && lock1==0){pass2=varcode;}
                             
                             
                             if(m==4){if((varcode&0xfffe)==(pass3&0xfffe)){
                                                                              
                                                                              if(varcode & 1){lock2=0;maxcomponent=0;}else {pass3=passcun&0xfffe;lock2=1;maxcomponent=0;}
                                                                              temp=pass3^0xa55;
...
if(numin)var2[page-2][line2]=temp;
//temp=0;
...
void r56process(void){if(page==1)page=0;else page=1;}
...

{
 ...
 number=10;
 if(keycode==18){number=1;/* //streadflag=0;/* //stTCCR0=0x00;*/}
 if(keycode==26){number=2;read(1);}
if(keycode==34){number=3;read(2);}
 if(keycode==17){number=4;}
 if(keycode==25)number=5;
 if(keycode==33)number=6;
 if(keycode==16)number=7;
 if(keycode==24)number=8;
 if(keycode==32)number=9;
 if(keycode==35)number=0;
 if(keycode==27 )selectprocess();
 if(keycode==31)escprocess();
 if(keycode==23)
    ...
    if(line2>0)line2--;else line2=dc[page]-1;
 ...
 {keywait=70; 
 downprocess();    
  line2++;if(line2>=dc[page])line2=0;
  ...
  {                  
    keywait=70;
  line++;if(line>=13)line=0;
   if(page==2)
     {
     if(line2<16)line2+=4; else if(line2<20) line2=20; else line2=0; 
      }
      else
   if(page==3)
     {
     if(line2<16)line2+=4; else if(line2<20) line2=20; else line2=0; 
        ...
        if(page==4)
     {
     if(line2<9)line2+=4; else 
       if(line2<12)line2=12;
       else
       if(line2==12) line2=13; else line2=0; 
      }
      else            
     if(page==5)
     {
    if(line2==9 || line2==10 || line2==8)line2++;
     else
     if(line2<4)line2+=4; else if(line2<8) line2=8; else line2=0; 
      }
      else
     if(page==6)
     {

     if(line2<12)line2+=4; else if(line2<16) line2=16; else line2=0; 
    ...
    if(line2==9 || line2==10 || line2==8)line2++;
     ...
     if(line2<4)line2+=4; else if(line2<8) line2=8; else line2=0; 
      ...
      if(page==8 ||page==9)
      {
     if(line2<13)line2+=4; else 
       if(line2<16)line2=16;
       else
       if(line2==16) line2=17; else line2=0; 
     ...
     if(line2<16)line2+=4; else  line2=0; 
      ...
      {
      line2++;if(line2>=dc[page])line2=0;lineprocess();
      }
    
      while(!var_valid[page*29+line2])
       {
        line2+=4;
        if(line2>=dc[page])line2=0;
 ...
 if(keycode==20)
  {                  
  keywait=70;
  if(line>0)line--;else line=12;
  
   if(page==2)
    {                
     if(line2>3)line2-=4; else line2=20; 
    }else
   if(page==3)
    {                
     if(line2>3)line2-=4; else line2=20; 
   ...
   if(page==4)
    {                                                     
     if(line2==13)line2=12;
      else
      if(line2>3)line2-=4; else line2=13; 
    }else
   if(page==5)
    {                 
    if(line2==9 || line2==10 || line2==11)line2--;
    else
     if(line2>3)line2-=4; else line2=11; 
    }else
      if(page==6)
    {                
     if(line2>3)line2-=4; else line2=16; 
    }else
   ...
   if(page==7)
    ...
    if(page==8 || page==9)
    {
     if(line2==17)line2=16;
      else
      if(line2>3)line2-=4; else line2=17; 
   ...
   if(page==10)
   {
     ...
     {
         if(line2>0)line2--;else line2=dc[page]-1;lineprocess();
 ...
 if(keycode==30)r13process();
if(keycode==40){govein=1;}
 if(protection)
 {
 if(keycode==8){pmc=1;state2=2;timer2=var2[0][8];timer=var2[0][12];yekst=1;}
 if(keycode==0){pmo=1;state2=8;timer=var2[1][4];}
 if(keycode==9  ){pub=1;state2=7;}//r23
 if(keycode==1){puf=1;timer=7;state2=3;}//r26
 if(keycode==2){pri=1;ritime=0;ristate=0;state2=4;}//r26
 if(keycode==3){pbri=1;state2=5;}//r26
 if(keycode==10){if(var[3][8]){pmri=1;state2=6;}else errorcode=2;}//r26
 if(keycode==5 ){ if(var2[6][17] ){pepm=1;state2=10;}else errorcode=2;} //r26
 if(keycode==13 ){if( var[6][7] ){pco=1;timer=var[6][5];state2=11;}else errorcode=2;}//r26
 if(keycode==11 ){if( var[5][8] ){peb=1;timer=var[5][5];state2=13;}else errorcode=2;}//r26

 if(keycode==14 ){if( var[7][8] ){pgb=1;state2=9;}else errorcode=2;}
 if(keycode==4 ){if(var[5][8] ){pef=1;state2=12;timer=var2[5][0];}else errorcode=2;}
// if(keycode==13){peb=1;state2=12;}
// if(keycode==6)pco=1;
 if(keycode==6 ){if( var2[6][16]){pci=1;timer=var[6][2];state2=1;}else errorcode=2;}
if(keycode==12){ if(var2[6][17] )darbaz=1;else errorcode=2;}
if(keycode==5){ if(var2[6][17] )darbasteh=1;else errorcode=2;}
 if(keycode==31 ){govein=1;}
 

} 
if(keycode==7){if(semiauto || automatic &&  !state )auto3=1;}//r26

 if(keycode==29 && var2[9][5] && maxcomponent<99998 ) {if(moto && hitteron && !i24){oil=0; if(i19){oil=oil;automatic=1;protection=0;ehteyatkey=0;}else {errorcode=18;moto=0;}}else {if(!moto || i24)errorcode=1;if(!var2[9][5])errorcode=2;if(!hitteron)errorcode=5;}}  
 if(keycode==21 && maxcomponent<99998){ if(moto && hitteron && !i24){oil=0;if(i19){oil=oil;semiauto=1;state=0;finish=semiauto;protection=0;ehteyatkey=0;}else{moto=0; errorcode=18;}}else {if(!moto || i24)errorcode=1;if(!hitteron)errorcode=5;}}
 if(keycode==22){moto^=1;if(moto && !i19){moto=0;errorcode=18;}}
 if(keycode==37){if(oil)oil=0;else {if(protection)oil=1;}}
 if(keycode==38){oil=oil;protection=1;semiauto=0;automatic=0;if(!i19) errorcode=18;}
 if(keycode==30)hitteron^=1;
if(keycode==36) { ehteyatkey^=1;if(!protection)ehteyatkey=0;}
if(keycode==15) { if(oil)oilcycle=0; else if(automatic || semiauto || protection) {oiltime=0;oilcycle=1;}}

  // if(page!=0)
 {
 if(keycode==49){page=9;line2=0; temp=0;}
 if(keycode==42){page=3;line2=0; temp=0;}
 if(keycode==43){page=6;line2=0; temp=0;}
 if(keycode==44){page=5;line2=0; temp=0;}
 if(keycode==50){page=13;line2=0; temp=0;}
 if(keycode==51){page=14;line2=0; temp=0;}
 if(keycode==52){page=11;line2=0; temp=0;}
 if(keycode==53){page=12;line2=0; temp=0;}
 if(keycode==41){page=2;line2=0; temp=0;}
 if(keycode==45){page=4;line2=0; temp=0;}
 if(keycode==46){page=7;line2=0; temp=0;}
 if(keycode==47){page=8;line2=0; temp=0;}
 if(keycode==48){page=10;line2=0; temp=0;}
  }
                             ...
                             if(chanel==c4051){
                                                        //st    in[c4051]=ADCW;
                                                         ...
                                                         //st  if(c4051==3){PORTF=0; 
																														if(errorcode && !(secend%3))setout20; else clrout20;//st}                         
...
/*                              if(chanel==3){PORTF=0;}                         
 ...
 if(timebase==100){if(brightcun)brightcun--;cheshmak++;if(timestart)totaltime++;secbase++;oiltime++;ritime1++; ritime++;timebase=0;if(timer)timer--;if(timer2)timer2--;if(timer1>1)timer1--;} 
  ...
  //ruler1=2000;
   ...
   if(tempp==1)
      ...
      staten=state;
//write_ppi(0x80,0x33);
        if(speed>speed2)if(!kkkk--){kkkk=200;speed2=speed;}
        if(speed<speed2)speed2=speed;
                        if(speed==0)speed2=0;
      ...
      if(outputa || outputb || (outputc & 0xf3) ) write_ppi(0xfe,3);else write_ppi(0xff,3);
     //st  PORTA=led ;
      //st PORTG=4;
     //st  PORTG=0;
     keyscan();
      ...
      if(keypress)
                      {refresh3=1;keypross();keypress=0;    if(protection )if(!lockkey)errorcode=0;  }
       
     if(!cycleex)if(!semiauto2){if(automatic && auto3){semiauto2=1;state=0;finish=1;}}
                     //taghir

    if(fffk)
    {   
      if(!i8 && oldi8){if(var[9][8])if(semiauto || automatic){auto4=1;fffk=0;}oldi8=i8;}else oldi8=i8;
   }       
   else
   {
    oldi8=i8;
...
{
#define localecho 1
	char string[10],i,j,commandstr[15],arg[4][10];
	u16 index,datain;
float number;
for(i=0;i<30;i++)
{	
...
if(strcmp(commandstr,"update_group")==0)
   {
   for(i=0;i<atoi(arg[0]);i++)update_var(i,atoi(arg[1]));
...
if(command[index] =0x3b){echo"; detected");
    sendecho

for(i=0;i<4;i++)for(j=0;j<10;j++)arg[i][j]=0;//debug 5
...
if(bufpointer<=bufsize){USART_SendData(USART2,lcdbuf[bufpointer]);bufpointer++;transmit=1;} else {transmit=0;colorm();}
	...
	void USART1_IRQHandler(void)
{
 ...
 // USART_SendData(USART1,'t');
	...
	if(command_index>28)command_index=0;
...
u16 i=0,j=0,counter;
	...
	if(address==addresspage+0xc00-8){memload();vartomem(1);FLASH_ErasePage(addresspage+0xc00);}
	if(address==addresspage+0x1000-8){memload();vartomem(1);FLASH_ErasePage(addresspage+0x800);}
...
u16 i=0,datain=0,datainmin=0xffff;
 FLASH_Unlock();
 findvalidpage();
	if(var_page_address == 0xffffffff)var_next_address=addresspage;else
	                                  {
																			if(var_page_address ==addresspage)var_next_address=addresspage+0x400;
																			                             else var_next_address=addresspage;
																		}
...
FLASH_Unlock();
	...
	if((*(vu16*)(addresspage)==0xffff) && (*(vu16*)(addresspage+0x400)==0xffff)){pageaddressmax=0xffffffff;echo"1st page avaliable");sendecho;}
                                                        else
																												{
                                                          if ((*(vu16*)(addresspage)==0xffff))pageaddressmax=addresspage+0x400;
																													else{
																													if ((*(vu16*)(addresspage+0x400)==0xffff))pageaddressmax=addresspage;
																													else {
																													if((*(vu16*)(addresspage))> (*(vu16*)(addresspage+0x400)))
																													 pageaddressmax=addresspage;else pageaddressmax=addresspage+0x400; 
																												       }
																														 }																											
																												}																													
...
void vartomem(u16 flag)// flag=0 then close page .
{
  ...
  echo"\n vartomem:counter = %x ,address= %x ",counter,address);
	...
	if(address != 0xffffffff)counter=*(vu16*)(address);// if address invalid micro halt
  echo"\n vartomem:loaded counter = %x ,address= %x ",counter,address);
	...
	if(!flag)update_var(0xfffe,1); // make a new change page
...
u16 get_var(u32 var_address)
{
	...
	sendecho
	FLASH_Unlock();
	if(*(vu16*)(addresspage+0xc00-8)==0xffff   && *(vu16*)(addresspage+0x1000-8)==0xffff)// if  end of both page is clear 
 	{
		//oft0.0 if(*(vu16*)(addresspage+0x800)!=0xffff)end_address=addresspage+0xc00-8;else end_address=addresspage+0x1000-8;
   	if(*(vu16*)(addresspage+0x800)/*page 2*/!=0xffff)end_address=0xc00-8;else end_address=0x1000-8;//aft0.0
		echo"\n get_var#2 : end address= %x",end_address);
	sendecho	
	}	else 
	if(*(vu16*)(addresspage+0xc00-8)==0xffff )end_address=0xc00-8; else end_address=0x1000-8;
	
//	
   end_address=end_address+addresspage;
	echo"\n get_var#3 : end address= %x",end_address);
	sendecho
	for(address=end_address;address>(end_address-0x400);address=address-8)
	{
...
char string[100];
  u16 i,j,counter;
	...
	{
		sprintf(string,"\n memload:variables set to defualt and write to address = %x  ",var_page_address);
	send_string(string);
...
char string[100];
  u16 i,j,counter;
	...
	if(page>PageLimit)return 0;
	address=addresspage+0x400*(page+6);
if(*(vu16*)(address)==0xffff)return 0;
...
#define IncPage addresspage+0x1000
	...
	echo"\ninc#2 : i=%u ,counter=%u, j= %u,addr=%x",i,j,counter,address);
		//sendecho
		if(i !=0xffffffff)address+=4;
	if(address>=(IncPage+0x400-4)){counter=i;FLASH_ErasePage(IncPage);address=IncPage;i=0xffffffff;echo"\ninc overpage: i=%x ,counter=%d, address= %x",i,counter,address);sendecho}
	} while(i!=0xffffffff);
...
*/
        if(autokey){lockkey=0;autokey=0;}
        for(i=0;i<5;i++);
        //strow=PINA;
        //row=1;//aft
				if(row ^0xff)
...
if(semiauto2)
  {
     if(finish  )
      ...
      // {imagekey=255;}
       ...
       {                                                   
        ...
        if (state==5 && mavad2){mavad2=0;state=3;}
        if(state==0)  {if(var2[3][8])mavad2=1;timer=0;totaltime=0;timestart=1;if(var2[5][10]){ pnum=var2[5][10]-1;}else pnum=0; if( var2[6][16] ){lockkey1=1;pci1=1;timer=var[6][2];}else state++;}
        if(state==1)  {if(LMC){timer2=var2[0][8];lockkey1=1; pmc1=1;temp111=1;yekst=1;}else state++;}
        if(state==2)  {lockkey1=1;puf1=1; }
        if(state==3)  {if(mavad2)state++; else {lockkey1=1;pri1=1;ritime=0;} }
        if(state==4)  {lockkey1=1;pbri1=1;ritime=0;}
        if(state==5)  {clrout3;{lockkey1=1;pmri1=1;} tempp=0;}
        if(state==6)  {if(var[2][8]){timer=var[2][6];lockkey1=1;pub1=1;}else state++;  }
        if(state==7)  {yekst=1;timer=var2[1][20]*10;lockkey1=1;pmo1=1;if(lock2 && maxcomponent<1000000) maxcomponent++;ccunter++;oilnumber++;}
        if(state==8)  {fffk=1;if(var[7][8]){lockkey1=1;pgb1=1;ritime1=0;}else state++;   }
        if(state==9)  {fffk=1;if(var2[6][17]){lockkey1=1;pepm1=1;}else state++; }
        if(state==10){fffk=1;if(var[6][7]){lockkey1=1;pco1=1;timer=var[6][5];}else state++;  }
        if(state==11){fffk=1;if(var[5][8]){lockkey1=1; pef1=1;timer=var2[5][0];}else state++;  }
        if(state==12){fffk=1;if(var[5][8]){lockkey1=1;peb1=1;timer=var[5][5];}else state+=2;}
    ...
    //    timer=60;                                                 
       ...
       state2=state;
       if(state>=13){
                            if(pnum){pnum--;state=11;state2=13;}
                            } 
       
        if(state==14 && timefull>=var[9][2] ){clrout13;tempp=0;timefull=0;state=0;state2=0;lockkey1=0;semiauto2=0;timestart=0;timep=totaltime;if(timep)tedad=36000/timep;}//semiauto=0;
        if(state!=14)finish=0;
       
        autokey2=0;
        if(state<14 || !(timefull%10))refresh2=1;
        }
  ...
  }else{finish=0;state=0;imagekey=0xff;if(!protection){timer=0;}clrout13;clrout9;clrout24;clrout23;tempp=0;clrout3;clrout17;clrout14; pci1=0;pmc1=0;puf1=0;pri1=0;pbri1=0;pmri1
  =0;pub1=0;pmo1=0;pgb1=0;pepm1=0;pco1=0;pef1=0;peb1=0;}
        //imagekey=cycl}
...
void SendDataDac(unsigned long int d)
{
unsigned int i;
	DACCS_HIGH
delay_us(10);
	...
	//delay_us(1500);
for(i=0x800000;i!=0;i=i>>1)
	              {
if(d & i)DACDATA_HIGH else DACDATA_LOW 
...
//st   UDR0=0;             
  if(maxcomponent != 0)maxcomponent=0;
page=0;                                   
alaki=2000;
write_ppi(0x80,0x33);
write_ppi(0x55,0x30);
...
ftable();
memactive=1;
oilnumber=0;oilcycle=1;oiltime=0;
if(pass1==3333)lock=1;
oil=oil;protection=1;semiauto=0;automatic=0;moto=0;hitteron=0;if(!i19) errorcode=18;
oil=oil;protection=1;semiauto=0;automatic=0;if(!i19) errorcode=18;
...
SendDataDac(0X690F00);
SendDataDac(0X810000);
...
while(1)
{
      ...
      // `temp=0;
       ...
       if(var2[7][16]!= oil)var2[7][16]=oil;
       if(maxcomponent>120000)maxcomponent=0;
      if(!i8)
        {
        if(errorc<210) errorc++;
        ...
        errorc=0;
        if(errorc>200)errorcode=6;
               
      if(page)
      {
       if(page !=9)if(temp>999){if(page!=11)temp=0;if(!lockkey && !lockkey1)monitorn();}
   ...
   /*     if(readflag>0){
    ...
    //    if(blocknumber>=(firstblock[0]+(dx)))blocknumber=firstblock[0];
      ...
      */        
      if(page==13 ||page==14)if(!lockkey && !lockkey1)monitorn();
     if(keycode==20 || keycode==28 )if(lockkey)if(keywait){keywait--;if(!keywait)lockkey=0;}     
      if(page==2 || page==3)
      {
      ...
      if(!page)
      {
                                   ...
                                   putdigit_xy_space(8,6,speed,0,2);     
                                    if(state2==4 ||state2==3 || state2==5) putdigit_xy(1,37,ritime,1);
                                      
                                  else  if(state2==14) putdigit_xy(1,37,timefull,0);
                                   else putdigit_xy(1,37,timer,1);

                                            putdigit_xy_space(1,20,ruler1m,0,3);     
                                  putdigit_xy_space(16,20,ruler2m,0,3);     
                                  putdigit_xy_space(16,37,timemax,1,3);     

      if(errorcode && !page)showpic_xy(errorcode,0,0);
           //if(temp)showpic_xy(temp,0,0);
      else if(state2==8 &&timer){} else {if(state2 && !page && state2<14)  {showpic_xy(25+state2,15,0); if(state2==4 && memseg<4)putdigit_xy(20,6,memseg,0);}                                     }
      ...
      if(oil)state2=0;
      if(protection && !lockkey && state2 ){state2=0;monitorn();}
      gosfand=var2[3][0]*10;      
...
//           protection=0;semiauto=0;automatic=0;oil=1; }      
    ...
    {   
       ...
       if(!LGR && oldlgr)  {if(var[9][8])if(semiauto || automatic){auto4=1;fffk=0;}oldlgr=LGR;}else oldlgr=LGR;
       if(!LGF && oldlgf)  {if(var[9][7])if(semiauto || automatic){auto4=1;fffk=0;}oldlgf=LGF;}else oldlgf=LGF;
     }else
       {
        oldlgr=LGR;
        oldlgf=LGF;
...
//       ruler2=in[1];                          
  //     ruler1=in[0];
       }else
       {
        ...
        {
       // if(ruler1=var[0][2]+1;
        if(!i2)ruler1a=ruler1p1-1;
        if(i3old && !i3)ruler1a= ruler1p2-1;  //labeyetahriki3
        if(!i3old && i3)ruler1a=ruler1p3-1;
//        i3old=i3;
        ...
        if(output & 2)
        {
          if(!i5)ruler1a=ruler1p5-1;
          if(!i4)ruler1a= ruler1p6+1;
...
i3old=i3;       
      ...
      var2a=(cul/1023)*ruler2;
        ...
        // per=perv1;
         ...
         if(!lockkey)hitreal=0;
  // if(!timer && !finish){timer=10; finish=1;}      
 
//error sathroghan                                                                       
  if(!i21)clrout24;
     if(!i19){moto=0;if(automatic ||semiauto){automatic=0;semiauto=0;errorcode=18;}   }
//roghankari
   if(oilcycle ) { if(oiltime>var[9][1]*10){clrout19;oilcycle=0;}else setout19;}else clrout19;   
if(oilnumber==var[9][0]){oilnumber=0;oilcycle=1;oiltime=0;}
//ghateshomar
if(ccunter>var[9][3]){if(oil)errorcode=21;} 
//full auto
if(i24 && (automatic || semiauto) ){automatic=0;semiauto=0;protection=1;} 
//saat kar
 if(secbase>=10){
//                              if(!beepcounter)beep=0;else beepcounter--;
                               ...
                               if(!i24)secend++;
                               secbase=0;
                               if(state==14)timefull++;else timefull=0;
  //                            monitorn();
//                              monitorn();
                             //  if(statetotaltime
                               if(errorcode){beep=1;beepcounter=40;}
                               ...
                               secend=0;                   
                                   ...
                                   hour++;                  
                                   hour1++;
                                   if(!lock2)maxcomponent+=12;
                                   
                                   
                                  // var[9][5]++;   
                                  // tempgo=var2[4][9];
                                    if( hour == var2[9][5]  ){errorcode=19;protection=0;semiauto=0;automatic=0;oil=oil;}
                                      if(hour>var2[9][4] )hour=0;
                                
                                
                                }
                               }
 
                                    if(hour==var2[9][4] && oil)errorcode=19;
//   khataye taviz roghan
       if(automatic)semiauto=1;
     if(auto3)
     {
       if(semiauto && !automatic){semiauto2=1;auto3=0;finish=1;state=0;lockkey1=0;timer=0;}
     
     }                                       
     if(!automatic && !semiauto)auto3=0;
 /*  if(auto3 )
    {
   ...
   */
if(!semiauto)semiauto2=0;
 if(alaki2<0)
    {                                                         
       if(hitteron)tempctl();//st else PORTE=0xff;
       alaki2=100;
       avrage[0]+=in[3];
       avrage[1]+=in[4];
       avrage[2]+=in[5];
     ...
     {
                 calibration();
      if(page==0 && !lockkey && !lockkey1)monitorn();
      cun2=50;
      tempreture[0]=(avrage[0]/50)+t1cul-200;
       if(tempreture[0]>1500)tempreture[0]=0;
      tempreture[1]=(avrage[1]/50)+t2cul-200;
      if(tempreture[1]>1500)tempreture[1]=0;
      tempreture[2]=(avrage[2]/50)+t3cul-200;
      if(tempreture[2]>1500)tempreture[2]=0;
      tempreture[3]=(avrage[3]/50)+t4cul-200;
      if(tempreture[3]>1500)tempreture[3]=0;
      tempreture[4]=(avrage[4]/50)+t5cul-200;
       if(tempreture[4]>1500)tempreture[4]=0;
        avrage[0]=0;   
        avrage[1]=0;   
        avrage[2]=0;   
        avrage[3]=0;   
      ...
      alaki--;                                            
      if(protection  || oil)if(!lockkey  && errorcode){errorcode=0;monitorn();}     
      if(!alaki){
                    
         alaki=501;temprecive=5;
         if(page==0)refresh=1;
         ...
         // if(keypress)
          ...
          //if(keypress)
            //            {refresh=1;keypross();monitorn();keypress=0;    if(protection )if(!lockkey)errorcode=0;  ftable();}
                       ...
                       if(automode)statecontroll();     
       
                cycleex=1;        cycle(); cycleex=0; 
      
       
      
       led=automatic*4+8*semiauto+128*moto+oil*2+protection*32+hitteron*64+ehteyatkey*16+ehteyatkey;
      if(page)limit();
      if(refresh){ monitorn();refresh=0;}else controll(); 
                  
      if(refresh2 && !page){refresh2=0;if(!lockkey && !lockkey1)monitorn();}
       
          if(refresh3){monitorn();refresh3=0;ftable();calibration();}
